...
spspmm_diag_sym_ABA(torch::Tensor rowptr, torch::Tensor col,
  ...
  if (rowptr.device().is_cuda()) {
// #ifdef WITH_CUDA
//     return spspmm_diag_sym_ABA_cuda(rowptrA, colA, optional_valueA, rowptrB, colB,
//                        optional_valueB, K, "sum");
// #else
    AT_ERROR("Not compiled with CUDA support");
// #endif
...
spspmm_diag_sym_AAAA(torch::Tensor rowptr, torch::Tensor col,
  ...
  if (rowptr.device().is_cuda()) {
// #ifdef WITH_CUDA
//     return spspmm_diag_sym_AAAA_cuda(rowptrA, colA, optional_valueA, rowptrB, colB,
//                        optional_valueB, K, "sum");
// #else
    AT_ERROR("Not compiled with CUDA support");
// #endif
...
torch::Tensor
spspmm_diag_ABCA(torch::Tensor rowptrAB, torch::Tensor colAB, torch::Tensor weightAB,
  ...
  if (rowptrAB.device().is_cuda()) {
// #ifdef WITH_CUDA
//     return spspmm_diag_ABCA_cuda(rowptrA, colA, optional_valueA, rowptrB, colB,
//                        optional_valueB, K, "sum");
// #else
    AT_ERROR("Not compiled with CUDA support");
// #endif
  } else {
    return spspmm_diag_ABCA_cpu(rowptrAB, colAB, weightAB,
...
static auto registry =
    torch::RegisterOperators()
        ...
        .op("sparse_tools::spspmm_diag_ABCA", &spspmm_diag_ABCA);
